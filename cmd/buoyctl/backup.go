...
func runContainerBackup(cmd *cobra.Command, containers []string) error {
	results, err := withSpinner(cmd, "Triggering backup...", func() ([]client.BackupResult, error) {
...
func runAllBackup(cmd *cobra.Command) error {
	results, err := withSpinner(cmd, "Triggering backup for all containers...", func() ([]client.BackupResult, error) {
...
func runProjectBackup(cmd *cobra.Command, project string, services []string) error {
	result, err := withSpinner(cmd, "Triggering project backup...", func() (*client.BackupResult, error) {
		return clientAPI().TriggerProjectBackup(project, services)
	})
	if err != nil {
		return fmt.Errorf("trigger backup: %w", err)
	}
	return renderBackupResults(cmd, []client.BackupResult{*result})
}

func renderBackupResults(cmd *cobra.Command, results []client.BackupResult) error {
	if len(results) == 0 {
